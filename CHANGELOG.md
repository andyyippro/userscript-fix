...
**新功能：DMM 预览视频（JavDB / JavBus）**

- JavDB 详情页点击「預告片」时，自动替换为 DMM 高清预览视频（拦截原生 fancybox）
- JavBus 详情页缩略图区域自动插入视频播放入口，仅当 DMM 实际有该番号预览视频时显示
- 支持 7 档画质切换：432p / 576p / 720p / 1080p / 1080p60 / 4K / 4K60，默认选最高可用画质
- 切换画质时保持当前播放进度
- 静音状态跨页面记忆（GM_getValue/GM_setValue）
- 视频弹窗全屏自适应（`calc(100vw - 80px)` / `calc(100vh - 80px)`），ESC 或点击遮罩关闭
- 页面加载后自动预加载视频地址，点击时即时播放；多次点击共享同一预加载 Promise，避免重复请求
- 视频搜索结果本地缓存，TTL 7 天，最多保留 100 条，写入时自动清理过期条目
- 自动过滤无 DMM 预览的番号类型（HEYZO、纯数字番号、VR 番号）
- 新增 `@connect api.dmm.com` 声明

## 1.7.0

- 新增标题翻译功能（日→中），支持 JavDB、JavBus 列表页和详情页。
- 使用 Google Translate API，翻译结果通过 GM_getValue/GM_setValue 持久缓存，避免重复请求。
- 设置面板新增"标题翻译"开关（默认开启）。
- JavDB 列表页：翻译后标题最多显示 3 行，日期左对齐，标签（含磁链/今日新种等）右对齐并保留 8px 右边距。
- JavBus 列表页：翻译后标题最多显示 2 行，标签/番号/日期始终可见不被截断。
- JavBus 列表页卡片布局优化：卡片自适应撑满容器，间距 8px，图片与文字间距缩小。
- JavDB 详情页：隐藏自带翻译和"顯示原標題"按钮，显示原标题 + 我们的翻译。
- JavBus 详情页：在标题下方显示翻译。
- 详情页翻译字体 20px，绿色左边框卡片样式。
- 无限滚动加载新内容时自动翻译。
- 新增 @connect translate-pa.googleapis.com 声明。
